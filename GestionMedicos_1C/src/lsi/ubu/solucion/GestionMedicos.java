...
public class GestionMedicos {
	...
	public static void reservar_consulta(String m_NIF_cliente, 
		...
		try{
			...
			pst_sel_consulta = con.prepareStatement(
					"select id_consulta, id_medico "
					+ "from "
					+ "(select * "
					+ "from consulta join medico "
					+ "on NIF = ? "
					+ "and medico.id_medico = consulta.id_medico "
					+ "and fecha_consulta = ?) c");
			...
			if(rs_sel_consulta.next()) {
				...
				pst_sel_anulacion = con.prepareStatement(
						"select id_anulacion "
						+ "from anulacion "
						+ "where id_consulta = ?");
			...
			if(rs_sel_cliente!=null) rs_sel_cliente.close();
			if(rs_sel_medico!=null) rs_sel_medico.close();
			if(rs_sel_consulta!=null) rs_sel_consulta.close();
			if(rs_sel_anulacion!=null) rs_sel_anulacion.close();
			if(pst_sel_cliente!=null) pst_sel_cliente.close();
			if(pst_sel_medico!=null) pst_sel_medico.close();
			if(pst_sel_consulta!=null) pst_sel_consulta.close();
			if(pst_sel_anulacion!=null) pst_sel_anulacion.close();
			if(pst_ins_consulta!=null) pst_ins_consulta.close();
			if(pst_upd_medico!=null) pst_upd_medico.close();
